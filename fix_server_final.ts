...
// I will just use string manipulation to remove from `/api/bububai/chat` to `/api/bububai/summarize`
const startIndex = content.indexOf('app.post("/api/bububai/chat", async (req, res) => {');
const endIndex = content.indexOf('app.post("/api/bububai/summarize"');

if (startIndex !== -1 && endIndex !== -1) {
  const updatedRoute = `app.post("/api/bububai/chat", async (req, res) => {
  try {
...
`;
  ...
  const newContent = content.substring(0, startIndex) + updatedRoute + content.substring(endIndex);
